...
impl PyHeistBatchEnv {
    ...
    pub fn get_render_state(&self, py: Python, env_idx: usize) -> PyResult<PyObject> {
        if env_idx >= self.inner.envs.len() {
            return Err(pyo3::exceptions::PyIndexError::new_err("env_idx out of bounds"));
        }
        let env = &self.inner.envs[env_idx];
        let dict = PyDict::new(py);
        dict.set_item("map_h", env.map_h)?;
        dict.set_item("map_w", env.map_w)?;
        dict.set_item("grid", env.grid.clone())?;
        dict.set_item("explored_map", env.explored_map.clone())?;
        dict.set_item("alarm", env.alarm)?;
        dict.set_item("alarm_max", env.alarm_max)?;
        dict.set_item("current_step", env.current_step)?;
        dict.set_item("terminal_disabled", env.terminal_disabled)?;
        dict.set_item("hack_progress", env.hack_progress)?;
        dict.set_item("loot_acquired", env.loot_acquired)?;
        dict.set_item("extraction_triggered", env.extraction_triggered)?;
        dict.set_item("extraction_countdown", env.extraction_countdown)?;
        dict.set_item("guard_positions", env.guard_positions.clone())?;
        dict.set_item("neutralized", env.neutralized.clone())?;

        let agent_pos_dict = PyDict::new(py);
        agent_pos_dict.set_item("scout", env.agent_positions[0])?;
        agent_pos_dict.set_item("hacker", env.agent_positions[1])?;
        agent_pos_dict.set_item("muscle", env.agent_positions[2])?;
        agent_pos_dict.set_item("extractor", env.agent_positions[3])?;
        dict.set_item("agent_positions", agent_pos_dict)?;
        dict.set_item("terminal_pos", env.terminal_pos)?;
        dict.set_item("loot_pos", env.loot_pos)?;
        dict.set_item("extract_pos", env.extract_pos)?;
