...
def train(
    ...
    for update in range(1, num_updates + 1):
        ...
        if do_crossover and active_experts < ECOOP_POOL_SIZE:
            ...
            # Compute empirical FIM from the rollout samples
            sample_obs = b_obs["scout"].reshape(-1, *OBSERVATION_SIZE)
            sample_role = b_role["scout"].reshape(-1, N_AGENTS)
            sample_mask = b_mask["scout"].reshape(-1, ACTION_SPACE_SIZE)
            sample_acts = b_actions["scout"].reshape(-1).long()

            fim = compute_fim_diagonal(
                agent.experts[best_expert], sample_obs, sample_role, sample_mask, sample_acts
            )
        ...
        if update % 5 == 0:
            ...
            msg = f"Update: {update}/{num_updates} | Win Rate: {win_rate:.2f} | Episodic Return: {mean_episodic_reward:.3f} | Step Reward: {avg_reward:.3f} | Active Experts: {active_experts}"
            print(msg)
            logging.info(msg)  # noqa: LOG015
