...
\caption{\textbf{Curriculum Scaling Performance.} Empirical win rate (\%) and episodic return across the five HEIST curriculum stages, evaluated over $N=1,\!000$ episodes per checkpoint across 10 random seeds ($10,\!000$ episodes per point, $350,\!000$ total episodes for our iterative lineage and MAPPO/H-MAPPO/COMA, plus $100,\!000$ episodes for ROMA and RODE under the identical protocol). Method lineage (MARC $\to$ COOP $\to$ E-COOP $\to$ THIEF) is shown in blue shades with final THIEF in crimson; MAPPO is shown in dark gray. Error bands denote $\pm$ one standard deviation across seeds. H-MAPPO and COMA are omitted from curves to preserve visual resolution; their full numbers appear in Table~\ref{tab:main_results}.}
...
To understand the mechanistic source of THIEF's performance advantage, we evaluate role-specific subtask metrics across curriculum stages (detailed in Table~\ref{tab:subtasks_stage3} and Figure~\ref{fig:subtasks}, Appendix~\ref{sec:app_subtasks}). Across all evaluated methods, early-phase subtasks are mastered robustly on Stage 3: Scout POI tagging exceeds $96\%$ for every method ($98.7\%$ or higher for all non-hierarchical methods) and Muscle guard neutralization exceeds $92.0\%$. However, the critical operational bottleneck occurs during sequential terminal override and payload extraction:
\begin{itemize}[leftmargin=*,nosep]
