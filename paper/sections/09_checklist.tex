...
\item {\bf Experiment statistical significance}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: All tables and figures report mean $\pm$ standard deviation over 10 independent training seeds, each evaluated over $1,\!000$ held-out episodes ($10,\!000$ episodes per condition, $450,\!000$ total evaluation episodes across all nine algorithms under the identical protocol). Section~\ref{sec:results} reports two-sided Welch's $t$-tests confirming statistical significance across all baseline comparisons on Stage 4 ($p < 10^{-3}$).
...
\item {\bf Experiments compute resources}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Section~\ref{sec:setup} and Appendix~\ref{sec:app_compute} document the hardware (NVIDIA GeForce RTX 3080 Ti GPU, Intel Core i9-10980XE 18-core/36-thread CPU), environment counts, per-stage training budgets, and measured throughput.
