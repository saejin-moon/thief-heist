...
% Light neutral panels with deep-blue accent for THIEF dataflow.
...
% Column 1: HEIST benchmark and roles
\node[rolebox] (scout) {
...
% Column 2: Decentralized router and specialist pool
...
\node[evobox, below=of plateau] (deficit) {
    \textbf{\footnotesize Deficit Gradient}\\[-1pt]
    \tiny failure transitions $\mathcal{D}_{\mathrm{fail}}$
};
...
\node[evobox, below=of fisher] (incubate) {
    \textbf{\footnotesize Expert Warmup}\\[-1pt]
    \tiny $25\%$ env.\ shards, $\tau_{\mathrm{iso}}=10$
};

\node[above=0.28cm of plateau, align=center] (col3_title) {
    \textbf{\small 3. Expert Spawning Engine}\\[-2pt]
    {\scriptsize\itshape dynamic warmup}
};
