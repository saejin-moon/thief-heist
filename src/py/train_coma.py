...
class ComaNetwork(nn.Module):
    ...
    def __init__(self, state_dim):
        ...
        self.critic = nn.Sequential(
            ...
            nn.Tanh(),
            layer_init(nn.Linear(128, ACTION_SPACE_SIZE), std=1.0),  # Outputs Q-values for all |U| actions
        )
    ...
    for update in range(1, num_updates + 1):
        ...
        for _epoch in range(UPDATE_EPOCHS):
            for a in AGENTS:
                ...
                v_loss = F.smooth_l1_loss(q_taken_new, ret_flat, beta=1.0)
                ...
                last_explained_var = (
                    (1.0 - torch.var(ret_flat - q_taken_new) / (var_y + 1e-8)).item()
                    if var_y > 1e-8
                    else 0.0
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="COMA Training")
    ...
    if save_dir is None:
        save_dir = f"results/coma/seed_{args.seed}/stage_{args.stage}"

    load_ckpt = args.load_ckpt
    if load_ckpt is None and args.stage > 0:
        prev_ckpt = f"results/coma/seed_{args.seed}/stage_{args.stage - 1}/model.pt"
