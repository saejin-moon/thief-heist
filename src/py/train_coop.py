...
class CoopNetwork(nn.Module):
    ...
    def get_action_and_value(
        self, obs, role, mask, goal, state, action=None, expert_idx=None
    ):
        ...
        if expert_idx is None:
            ...
            for expert in self.experts:
                val = expert.critic(x_critic).squeeze(-1)
...
def train(
    ...
    for update in range(1, num_updates + 1):
        ...
        if update % 5 == 0:
            ...
            curve_logger.log_update(
                ...
                active_experts=agent.num_experts,
                effective_experts=float(np.exp(0.0)),
                total_spawns=0,
                switch_rate=last_switch_rate if 'last_switch_rate' in locals() else 0.0,
                routing_entropy=0.0,
    ...
    role_expert_counts = np.zeros((N_AGENTS, agent.num_experts), dtype=np.float64)
    ...
    results_data = record_stage_completion(
        ...
        total_spawns=0,
        expert_switch_rate=float(last_switch_rate if 'last_switch_rate' in locals() else 0.0),
        routing_entropy=0.0,
...
if __name__ == "__main__":
    ...
    if save_dir is None:
        save_dir = f"results/coop/seed_{args.seed}/stage_{args.stage}"

    load_ckpt = args.load_ckpt
    if load_ckpt is None and args.stage > 0:
        prev_ckpt = f"results/coop/seed_{args.seed}/stage_{args.stage - 1}/model.pt"
