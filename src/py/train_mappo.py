...
class MappoNetwork(nn.Module):
    def __init__(self, state_dim):
        ...
        self.actor = nn.Sequential(
            layer_init(nn.Linear(obs_flat + N_AGENTS + GOAL_VECTOR_DIM, 64)),
            nn.Tanh(),
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Linear(state_dim + N_AGENTS + GOAL_VECTOR_DIM, 64)),
            nn.Tanh(),
            layer_init(nn.Linear(64, 64)),
            nn.Tanh(),
...
if __name__ == "__main__":
    ...
    if save_dir is None:
        save_dir = f"results/mappo/seed_{args.seed}/stage_{args.stage}"

    load_ckpt = args.load_ckpt
    if load_ckpt is None and args.stage > 0:
        prev_ckpt = f"results/mappo/seed_{args.seed}/stage_{args.stage - 1}/model.pt"
